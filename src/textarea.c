...
TextArea* textarea_create(const char* text, int maxTextSize, DFBRectangle* dimensions, TextAreaLooks* looks)
{
	assert(looks != NULL);
	...
	TextArea* tempArea = NULL;
	{
		...
		tempArea->looks.textFont->GetHeight(tempArea->looks.textFont, &tempArea->fontHeight);
	}
...
void textarea_draw(TextArea* this, IDirectFBSurface* surface)
{
	...
	do
	{
		if(this->looks.textFont->GetStringBreak(this->looks.textFont, text, -1, maxWidth, &retWidth, &retStrLen, &nextLine) != DFB_OK)
		{
			break;
		}

		DFBCHECK (surface->DrawString (surface, text, retStrLen, this->dimensions.x, yTextSpacing, DSTF_TOPLEFT));
