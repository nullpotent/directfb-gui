...
enum DFBEM_NUMBERS {DFBEM_0 = 0x04, DFBEM_1 = 0x06, DFBEM_2 = 0x03, DFBEM_3 = 0x08,
...
static void send_character(Dialog* this, char c)
{
	textarea_addchar(this->textArea, c);
}

void wait_sec_tosend(Dialog* this)
{
	numKeys--;
	if(numKeys==0)
	{
		send_character(this, alpha_table[indexTable][fieldCursor-1]+inputDif);
		fieldCursor = 0;
	}
}

void dialog_eventhandler(struct Dialog* this, DFBInputDeviceKeySymbol* sym)
{
	assert(this != NULL);
	firstKey = (*sym & 0xFF);
	if(firstKey >= DIKS_0 && firstKey <= DIKS_9)
	{
		...
		if(this->input_mode!=NUM_MODE)
		{
			...
			if((firstKey == DIKS_0 || firstKey == DIKS_9) && (this->input_mode == BIGALPHA_MODE)) {
				inputDif = 0;
			}

			if((firstKey != lastKey) && (numKeys > 0))
			{
				send_character(this, alpha_table[lastIndexTbl][fieldCursor-1]+inputDif);
				fieldCursor = 1;
			}
			else
				fieldCursor++;
...
Dialog* dialog_create(const char* name, DFBRectangle* dim, enum INPUT_MODE inputMode, DialogLooks* dlooks)
{
	assert(dim != NULL);
	assert(dlooks != NULL);
...
void dialog_draw(struct Dialog* this, IDirectFBSurface* surface)
{
	//blending required
	...
	DFBCHECK(surface->SetColor(surface,this->looks.backColor.r,this->looks.backColor.g,this->looks.backColor.b,this->looks.backColor.a));
	DFBCHECK(surface->FillRectangle(surface, this->dialogPos.x, this->dialogPos.y, this->width, this->height));

    //draw title bar
    DFBCHECK(surface->SetColor(surface,this->looks.titleColor.r,this->looks.titleColor.g,this->looks.titleColor.b,this->looks.titleColor.a));
	DFBCHECK(surface->FillRectangle(surface, this->dialogPos.x, this->dialogPos.y - this->looks.titleHeight, this->width, this->looks.titleHeight));

	//draw title bar text
	DFBCHECK(surface->SetFont(surface,this->looks.fontTitle));
    DFBCHECK (surface->SetColor (surface, this->looks.titleTextColor.r, this->looks.titleTextColor.g, this->looks.titleTextColor.b, this->looks.titleTextColor.a));
    DFBCHECK (surface->DrawString (surface, this->name,-1, this->dialogPos.x + this->width/2, this->dialogPos.y-this->looks.titleHeight/2, DSTF_CENTER));
