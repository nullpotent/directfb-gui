...
static void gui_eventhandler(DFBInputDeviceKeySymbol* sym)
{
	switch(*sym)
	{
		case DIKS_SPACE:
...
{
	dialog_draw(focusedDialog, DFBC_SURFACE_);
}

int gui_initcomponents()
{

	DFBRectangle rec = {100,100,800, 600};
	...
	create_font("lucid.ttf", &l.fontDsc, &l.fontTitle);
	...
	TextAreaLooks areaL;
	{
		...
		create_font("lucid.ttf", &areaL.textWindowFontDsc, &areaL.textFont);
	}

	DFBRectangle rec1 = {105, 105, 790, 590};
...
int gui_init()
{
	...
	DFB_EVHANDLER_FCTR = gui_eventhandler;
	DFB_RENDER_FCTR = gui_render;
